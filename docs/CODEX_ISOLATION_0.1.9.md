...
Development and testing use the current CLI's `--approve-for-me` mode, which provides its workspace-write sandbox and automatic approval review as one combined option. Read-only assessment continues to use `--sandbox read-only`. These options cannot be combined. Agent runs use the canonical user `CODEX_HOME` for authentication because refresh tokens rotate and must never be copied into competing stores. User configuration remains excluded by `--ignore-user-config`, sessions remain ephemeral through `--ephemeral`, and Studio keeps task schemas, results, logs, and run artifacts in its own application data directory. The resolver selects the newest compatible CLI so an older executable cannot be paired with a newer cache format.
